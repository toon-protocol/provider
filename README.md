...
What the config does not (yet) do: the per-lease `.anyone` addresses and
the proxied outbound land in the later Milestone 4 tickets against the
shapes defined here (the workload egress is done — the next section) — the
`HiddenService` port

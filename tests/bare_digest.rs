...
async fn a_bare_digest_runs_from_blob_records_with_no_entry_and_no_reference() {
    ...
    assert_eq!(
        looked_up,
        BTreeSet::from([index.clone(), manifest, config, layer])
    );
    assert!(h.directory.entry_lookups().is_empty());
    // And it ran: loaded by manifest digest, started by the id the load
    // produced, exactly as the entry form does.
    assert!(matches!(
...
async fn an_entry_record_whose_parts_fail_falls_through_to_one_on_the_relay_set() {
    ...
    assert!(
        paths.contains(&format!("/raw/{}-record", &layer[7..19])),
...
async fn a_gateway_5xx_on_one_part_moves_that_blob_on_and_leaves_the_others_alone() {
    ...
    assert!(requested.contains(&format!("/raw/{}-part1", &layer[7..19])));
    assert!(
        !requested.contains(&format!("/raw/{}-part2", &layer[7..19])),
